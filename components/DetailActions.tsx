...
export default function DetailActions({ workout }: { workout: Workout }) {
  ...
  function handleAdd() {
    ...
    if (result === "added") toast.success(`Added to today's plan`, { description: workout.name });
      ...
      toast.error("Already in today's plan", { description: `${workout.name} is added, check My Plan.` });
  ...
  function handleSave() {
    ...
    else toast.error("Already saved", { description: `${workout.name} is in your saved list.` });
  }
